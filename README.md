...
`tooltipIsHidden` – [boolean] определяет видимость всплывающей подсказки со значением. При значении `false` отображается всегда. При значении `true` всплывающая подсказка появляется только при изменении значения бегунка (по умолчанию: `true`)

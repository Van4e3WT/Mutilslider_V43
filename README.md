...
`tooltipIsHidden` – [boolean] определяет видимость всплывающей подсказки со значением. При значении `false` отображается всегда. При значении `true` всплывающая подсказка появляется только при изменении значения бегунка (по умолчанию: `false`)    
...
```html
  <div class="child js-multislider-v43 double slider-1" data-min-value="-1" data-max-value="1" data-step="0.01"
    data-value1="-0.25" data-value2="0.50" data-is-vertical data-is-range
    data-tooltip-of-value data-scale-of-values="9" data-is-progress-bar data-description="My Slider">
  </div>
```
